<?php
// db/migrate.php - Script Migrasi Otomatis & Pemutakhiran Database
require_once __DIR__ . '/../config/database.php';

echo "Memulai migrasi database...\n";

try {
    // 1. Pastikan kolom di tabel users lengkap
    $columns = $pdo->query("SHOW COLUMNS FROM users")->fetchAll(PDO::FETCH_COLUMN);
    
    if (!in_array('email', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN email VARCHAR(100) NULL UNIQUE AFTER username");
        echo "- Kolom 'email' ditambahkan ke tabel users.\n";
    }
    if (!in_array('no_hp', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN no_hp VARCHAR(25) NULL AFTER email");
    ...
    if (!in_array('avatar', $columns)) {
        ...
        echo "- Kolom 'avatar' ditambahkan ke tabel users.\n";
    }
    if (!in_array('status', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN status ENUM('aktif', 'nonaktif') NOT NULL DEFAULT 'aktif' AFTER avatar");
        echo "- Kolom 'status' ditambahkan ke tabel users.\n";
    }
    if (!in_array('reset_token', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN reset_token VARCHAR(100) NULL AFTER status");
        echo "- Kolom 'reset_token' ditambahkan ke tabel users.\n";
    }
    if (!in_array('reset_expires', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN reset_expires DATETIME NULL AFTER reset_token");
        echo "- Kolom 'reset_expires' ditambahkan ke tabel users.\n";
    }
    if (!in_array('updated_at', $columns)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
        echo "- Kolom 'updated_at' ditambahkan ke tabel users.\n";
    }

    // Perbarui ENUM role di tabel users agar mendukung 4 role: admin, bendahara, content_admin, donatur
    $pdo->exec("ALTER TABLE users MODIFY COLUMN role ENUM('admin', 'bendahara', 'content_admin', 'donatur') NOT NULL DEFAULT 'donatur'");
    echo "- ENUM role di tabel users diperbarui (admin, bendahara, content_admin, donatur).\n";

    // 2. Buat tabel-tabel baru
    $pdo->exec("CREATE TABLE IF NOT EXISTS profil_masjid (
    ...
    echo "- Tabel 'profil_masjid' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS pengurus_masjid (
    ...
    echo "- Tabel 'pengurus_masjid' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS rekening_donasi (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "- Tabel 'rekening_donasi' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS program_donasi (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "- Tabel 'program_donasi' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS kategori_transaksi (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "- Tabel 'kategori_transaksi' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS transaksi_keuangan (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "- Tabel 'transaksi_keuangan' siap.\n";

    $pdo->exec("CREATE TABLE IF NOT EXISTS donasi_online (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "- Tabel 'youtube_videos' siap.\n";
    ...
    echo "- Tabel 'pesan_kontak' siap.\n";

    // 3. Masukkan Seeder Data
    // Users
    $passwordHash = password_hash('admin123', PASSWORD_DEFAULT);
    $bendaharaHash = password_hash('bendahara123', PASSWORD_DEFAULT);
    $kontenHash = password_hash('konten123', PASSWORD_DEFAULT);
    $donaturHash = password_hash('donatur123', PASSWORD_DEFAULT);

    $stmt = $pdo->prepare("INSERT INTO users (id, username, email, no_hp, password, nama_lengkap, role, status) VALUES 
        (1, 'admin', 'admin@masjidnuruliman.id', '081234567890', ?, 'H. Ahmad Syukron, S.Ag (Ketua DKM)', 'admin', 'aktif')
        ON DUPLICATE KEY UPDATE role='admin', nama_lengkap='H. Ahmad Syukron, S.Ag (Ketua DKM)'");
    $stmt->execute([$passwordHash]);

    $stmt = $pdo->prepare("INSERT INTO users (id, username, email, no_hp, password, nama_lengkap, role, status) VALUES 
        (2, 'bendahara', 'bendahara@masjidnuruliman.id', '081234567891', ?, 'H. Muhammad Ridwan, SE (Bendahara)', 'bendahara', 'aktif'),
        (3, 'konten', 'media@masjidnuruliman.id', '081234567892', ?, 'Ust. Fajar Ramadhan (Divisi Media)', 'content_admin', 'aktif'),
        (4, 'donatur', 'donatur@gmail.com', '081234567893', ?, 'Bpk. H. Bambang Subagyo', 'donatur', 'aktif')
        ON DUPLICATE KEY UPDATE role=VALUES(role), nama_lengkap=VALUES(nama_lengkap)");
    $stmt->execute([$bendaharaHash, $kontenHash, $donaturHash]);
    echo "- Akun default (admin, bendahara, konten, donatur) berhasil dikonfigurasi.\n";

    // Profil Masjid
    $countProfil = $pdo->query("SELECT COUNT(*) FROM profil_masjid")->fetchColumn();
    if ($countProfil == 0) {
        $pdo->exec("INSERT INTO profil_masjid (id, nama_masjid, sebutan, slogan, sejarah, visi, misi, alamat, kota, google_maps_embed, whatsapp, email, instagram, youtube, facebook, saldo_awal_kas) VALUES
        ...
        'Menjadi pusat peradaban Islam yang mandiri, makmur, berakhlak mulia, serta unggul dalam pelayanan ibadah jamaah dan pemberdayaan sosial ekonomi ummat.',
        '1. Menyelenggarakan ibadah fardhu dan sunnah secara berjamaah dengan tertib, bersih, nyaman, dan khusyuk.\n2. Mengembangkan majelis taklim, tahsin, tahfidz Al-Qur\'an, dan kajian keislaman kontemporer untuk seluruh kalangan usia.\n3. Mengelola dana zakat, infaq, sedekah, dan wakaf (ZISWAF) secara amanah, transparan, dan profesional berbasis teknologi informasi.\n4. Menyelenggarakan program santunan sosial berkala, pembinaan generasi muda Islam, dan kepedulian dhuafa.',
        ...
        echo "- Data profil masjid berhasil disimpan.\n";
    }

    // Pengurus DKM
    $countPengurus = $pdo->query("SELECT COUNT(*) FROM pengurus_masjid")->fetchColumn();
    if ($countPengurus == 0) {
        $pdo->exec("INSERT INTO pengurus_masjid (id, nama, jabatan, bidang, no_hp, urutan) VALUES
        (1, 'H. Ahmad Syukron, S.Ag', 'Ketua Umum DKM', 'Pimpinan Utama', '081234567890', 1),
        (2, 'Drs. H. Abdul Wahab, M.Pd.I', 'Wakil Ketua DKM', 'Pimpinan', '081234567895', 2),
        (3, 'Ust. H. Mahfuzh Amin, Lc', 'Sekretaris Umum', 'Kesekretariatan & Dakwah', '081234567896', 3),
        ...
        (6, 'Bpk. Suparman', 'Koordinator Pemeliharaan', 'Sarana & Marbot', '081234567897', 6)");
        echo "- Data pengurus DKM berhasil diinisialisasi.\n";
    }

    // Rekening Donasi
    $countRekening = $pdo->query("SELECT COUNT(*) FROM rekening_donasi")->fetchColumn();
    if ($countRekening == 0) {
        $pdo->exec("INSERT INTO rekening_donasi (id, nama_bank, nomor_rekening, atas_nama, kategori_donasi, urutan) VALUES
        ...
        echo "- Data rekening resmi masjid berhasil diinisialisasi.\n";
    }

    // Program Donasi
    $countProgram = $pdo->query("SELECT COUNT(*) FROM program_donasi")->fetchColumn();
    if ($countProgram == 0) {
        $pdo->exec("INSERT INTO program_donasi (id, nama_program, slug, kategori, deskripsi, deskripsi_lengkap, target_donasi, dana_terkumpul, tanggal_mulai, tanggal_selesai, status, is_featured) VALUES
        (1, 'Pembangunan Menara & Perluasan Tempat Wudhu', 'pembangunan-menara-dan-tempat-wudhu', 'operasional', 
        'Renovasi dan perluasan fasilitas tempat wudhu ramah lansia serta penyelesaian lantai 2 menara kumandang adzan.',
        ...
        echo "- Data katalog program donasi berhasil diinisialisasi.\n";
    }

    // Kategori Transaksi
    $countKategori = $pdo->query("SELECT COUNT(*) FROM kategori_transaksi")->fetchColumn();
    if ($countKategori == 0) {
        $pdo->exec("INSERT INTO kategori_transaksi (id, jenis, nama_kategori, deskripsi, urutan) VALUES
        ...
        echo "- Data kategori kas berhasil diinisialisasi.\n";
    }

    // Transaksi Keuangan
    $countTrx = $pdo->query("SELECT COUNT(*) FROM transaksi_keuangan")->fetchColumn();
    if ($countTrx == 0) {
        $pdo->exec("INSERT INTO transaksi_keuangan (no_transaksi, tanggal_transaksi, jenis, kategori_id, program_id, akun_kas, nominal, keterangan, user_id, metode_pembayaran, is_published) VALUES
        ...
        echo "- Data transaksi pembukuan berhasil diinisialisasi.\n";
    }

    // Donasi Online
    $countDonasiOnline = $pdo->query("SELECT COUNT(*) FROM donasi_online")->fetchColumn();
    if ($countDonasiOnline == 0) {
        $pdo->exec("INSERT INTO donasi_online (no_donasi, user_id, nama_donatur, is_anonim, email, no_wa, program_id, nominal, metode_pembayaran, bank_tujuan, doa_donatur, status, tanggal_donasi) VALUES
        ...
        echo "- Data donasi online berhasil diinisialisasi.\n";
    }

    // Berita & Artikel
    $countBerita = $pdo->query("SELECT COUNT(*) FROM berita")->fetchColumn();
    if ($countBerita == 0) {
        $pdo->exec("INSERT INTO berita (id, judul, slug, isi, ringkasan, kategori, penulis_id, tanggal_publikasi, status, views) VALUES
        ...
        echo "- Data berita masjid berhasil diinisialisasi.\n";
    }

    // Banners
    $countBanners = $pdo->query("SELECT COUNT(*) FROM banners")->fetchColumn();
    if ($countBanners == 0) {
        $pdo->exec("INSERT INTO banners (id, judul, subjudul, gambar, link_url, tipe, urutan, is_active) VALUES
        ...
        echo "- Data slider dan banner beranda berhasil diinisialisasi.\n";
    }

    // YouTube Videos
    $countVideos = $pdo->query("SELECT COUNT(*) FROM youtube_videos")->fetchColumn();
    if ($countVideos == 0) {
        $pdo->exec("INSERT INTO youtube_videos (id, judul, video_id, url_embed, kategori, deskripsi, is_active) VALUES
        ...
        echo "- Data video YouTube berhasil diinisialisasi.\n";
    }

    // Notifikasi
    $countNotif = $pdo->query("SELECT COUNT(*) FROM notifikasi")->fetchColumn();
    if ($countNotif == 0) {
        $pdo->exec("INSERT INTO notifikasi (id, user_id, judul, pesan, tipe, link) VALUES
        ...
        echo "- Data notifikasi awal berhasil diinisialisasi.\n";
    }

    echo "\n>>> SELURUH MIGRASI BERHASIL DILAKUKAN TANPA MASALAH! <<<\n";
